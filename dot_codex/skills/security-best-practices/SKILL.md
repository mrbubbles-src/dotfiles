...
For a specific dependency advisory, vendor security email, CVE, compromised package notice, supply-chain incident, or "are we affected?" question, also use the `dependency-advisory-triage` skill. That extension is the better fit for exact local impact checks before applying the broader framework guidance here.
